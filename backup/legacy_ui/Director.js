...
export const Logger = {
  ...
  logTurn(
    ...
    if (result && result.flipped) {
      console.log(`flip card: ${this.formatCard(result.flipped)}`);
    }
    ...
    const discards = game.state.turnDiscards || [];
    console.log(
    ...
    game.state.turnDiscards = [];
  },

  logSpecialPlay(player, card, type) {
    console.log(
...
export const Director = {
  ...
  async playOpeningCeremony(game) {
    game.startCeremony();
    ...
    let candidates = game.players.map((_, i) => i);
    ...
    SoundManager.play("flip");
    const startingCard = Actions.draw(game);
  ...
  setButtonsPulse(isActive) {
    // Select all action buttons that should wiggle
    const buttons = document.querySelectorAll(".btn-hl");
    buttons.forEach((btn) => {
      if (isActive) {
  ...
  async playTurn(game, choice) {
    ...
    const isHuman = game.currentPlayerIndex === 0 && !isSimulating;
    const handAtStart = player.hand
      .filter((c) => c !== null)
      .map((c) => ({ ...c }));
    const upCardAtStart = game.upCard ? { ...game.upCard } : null;

    // THE FIX: Capture a shallow copy of the discard registry before Engine processes it
    const registryAtStart = [...(game.discardPile || [])];
   ...
   // --- 3. DECK EXHAUSTION CHECK ---
    if (game.deck.length === 0 && game.discardPile.length > 0) {
      ...
      const remainingDiscards = game.discardPile.slice(0, -1);
    ...
    const turnMode = game.state.mode;

    // --- 4. QUEEN SOCIAL: No theme selection. Proceeds directly to color guess. ---

    const result = game.processGuess(choice);

    // --- 5. SPECIALIST HAND ACTION ROUTING ---
    if (
      result &&
      (result.type === "SABOTAGE_ACTIVATED" ||
        result.type === "SHIELD_ACTIVATED")
    ) {
      Logger.logTurn(
        game,
        player,
    ...
    if (result && result.type === "ACCEPT_START") {
      ...
      await this.playKingBountySequence(game, player, result);
      return;
    }

    // --- BOUNTY DECLINE FIX ---
    if (result && result.type === "DECLINED_ACTION") {
  ...
  async playKingBountySequence(game, player, result) {
    ...
    for (let i = 0; i < result.sequence.length; i++) {
      ...
      SoundManager.play("flip");
      game.state.lastFlippedCard = step.card;
      View.render(game);
      await sleep(TIMING.REVEAL);
      ...
      if (step.isMatch) {
        ...
        const failMsg = isLastFlip ? "NO MATCH! TAKE CARD." : "NO MATCH!";
        View.showBubble(
          game.currentPlayerIndex,
    ...
    if (finalFlippedCard) {
      game.state.lastFlippedCard = null;
      View.render(game);
  ...
  async executeRevealSequence(game, player, choice, result, turnMode) {
    ...
    const guessShout = guessShouts[String(choice)] || `${String(choice).toUpperCase()}!`;
    View.showBubble(pIdx, guessShout, false);

    const guessMsg = `${player.name} guessed ${choice.toString()}!`;
    View.renderMessageCenter(game, turnMode, null, guessMsg, game.deck.length + 1);

    // --- ANIMATION 1: DRAW FROM DECK ---
    await AnimationManager.flyDrawToCenter(result.flipped);
    SoundManager.play("flip");

    // Physical Flip (Transient Card sync)
    game.state.lastFlippedCard = result.flipped;
    View.render(game);
    ...
    // 2. RESULT PHASE
    if (result.type === "TIE" || result.type === "TIE_REGUESS") {
      ...
      if (!game.discardPile.includes(result.flipped)) {
        game.discardPile.push(result.flipped);
      }

      let successShout = turnMode === MODES.LUCKY_7 && result.type === "LUCKY_7_SUCCESS"
        ? "LUCKY DISCARD!" : pickReaction("CORRECT");
      ...
      const startRect = transientSlot ? transientSlot.getBoundingClientRect() : null;
      ...
      if (startRect) {
        await AnimationManager.flyTransientToHand(pIdx, result.flipped, startRect);
      }
    }

    View.renderMessageCenter(game, turnMode, null, result.message);

    const hasCombo = game.checkCombos(player);
    const activeCardCount = player.hand.filter((c) => c !== null).length;
    ...
    if (turnMode === MODES.ACE_STREAK) {
      if (result.type === "ACE_STREAK_COMPLETE") game.recordSpecialistStat("ace", "success");
    } else if (turnMode === MODES.LUCKY_7) {
      if (result.type === "LUCKY_7_SUCCESS") game.recordSpecialistStat("lucky7", "success");
    } else if (turnMode === MODES.SABOTAGE) {
      if (result.type === "SABOTAGE_FAIL") game.recordSpecialistStat("sabotage", "fail");
      else if (result.success) game.recordSpecialistStat("sabotage", "success");
    } else if (turnMode === MODES.KING_BOUNTY) {
      if (result.success) {
        game.recordSpecialistStat("bounty", "accepted");
        if (result.type === "BOUNTY_MATCH") game.recordSpecialistStat("bounty", "win");
      }
    }
    ...
    if (game.state.mustDiscard.length > 0 && !game.state.gameOver) {
      ...
      if (game.state.lastResult === UI_STRINGS.LUCKY_7_SUCCESS && !game.state.lastResult.includes("PAIR") && !game.state.lastResult.includes("STRAIGHT") && !game.state.lastResult.includes("FLUSH")) {
  ...
  async handleDiscardPhase(game) {
    ...
    if (isHuman) {
      ...
      while (
        ...
        const actualIndex = typeof discardData === "object" ? discardData.index : discardData;
        const cardToDiscard = typeof discardData === "object" ? discardData.card : game.discardPile[game.discardPile.length - 1];
        ...
        if (actualIndex !== undefined && actualIndex !== null) {
          ...
          const slotEl = playerBox?.querySelector(`.card-slot[data-index="${actualIndex}"]`);
      ...
      while (game.state.mustDiscard.length > 0) {
        ...
        if (targetRequirement === -1) {
          ...
          actualIndex = validIndices.sort((a, b) => {
            const distA = Math.abs(player.hand[a].rank - 7);
            const distB = Math.abs(player.hand[b].rank - 7);
            return distA - distB;
          })[0];
        } else {
        ...
        if (actualIndex !== -1 && player.hand[actualIndex]) {
          const cardToDiscard = player.hand[actualIndex];
          if (!game.state.turnDiscards) game.state.turnDiscards = [];
          game.state.turnDiscards.push(cardToDiscard);
  ...
  async handleHandAction(game, slotIndex, player) {
    const cardToPlay = player.hand[slotIndex];
    if (!cardToPlay) return;

    const isSabotage = cardToPlay.rank === 4;
    const isShield = cardToPlay.rank === 11;

    // Execute the logic in the engine
    const result = game.processGuess(slotIndex);
  ...
  advanceToNextActivePlayer(game) {
    ...
    const msg = game.state.lastResult || "";
    const isTransient =
    ...
    if (
      ...
      if (game.state.mode === MODES.KING_BOUNTY) {
        const activeCards = game.players[0].hand.filter(
          (c) => c !== null,
        ).length;
  ...
  async handleGameOver(game) {
    ...
    const isWinner = game.state.winner === humanPlayer;
    ...
    if (game.state.gameOver) {
      ...
      const finalWinMethod = game.state.bountyWinningCard
        ? "BOUNTY_INSTANT"
        : "LAST_MAN_STANDING";
