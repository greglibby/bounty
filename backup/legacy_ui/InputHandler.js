...
let isConfirmingNewGame = false;
let game;

function updateSettingsUI() {
  const speed = localStorage.getItem("BOUNTY_SPEED") || "1.0";
  const diff = localStorage.getItem("BOUNTY_DIFFICULTY") || "NORMAL";
  const isMuted = localStorage.getItem("BOUNTY_MUTED") === "true";

  // Use the .setting-active class for Gold styling
  document.querySelectorAll("#speed-options .nav-btn").forEach((btn) => {
    btn.classList.toggle("setting-active", btn.dataset.speed === speed);
  });

  document.querySelectorAll("#diff-options .nav-btn").forEach((btn) => {
...
const init = () => {
  ...
  document
    .querySelectorAll("#speed-options .nav-btn, #diff-options .nav-btn")
    .forEach((btn) => {
      btn.onclick = (e) => {
        e.stopPropagation();
        SoundManager.play("menu");
  ...
  if (soundBtn) {
    soundBtn.onclick = (e) => {
      e.stopPropagation();
      const currentlyMuted = localStorage.getItem("BOUNTY_MUTED") === "true";
      const newMuteState = !currentlyMuted;
      localStorage.setItem("BOUNTY_MUTED", newMuteState);
      updateSettingsUI();
      if (!newMuteState) {
        SoundManager.play("menu");
      }
    ...
    newGameBtn.onclick = (e) => {
      ...
      if (!isConfirmingNewGame) {
        ...
        setTimeout(() => {
          ...
          // 3. Clear overlays
          ["stats-overlay", "records-overlay", "rules-overlay"].forEach(
            (id) => {
  ...
  document.addEventListener("click", async (e) => {
    ...
    if (actionBtn) {
      ...
      const choice = actionBtn.dataset.theme || actionBtn.dataset.guess;
      if (choice) {
    ...
    if (cardSlot) {
      ...
      const slotIndex = parseInt(cardSlot.dataset.index);
      ...
      if (game.state.mustDiscard.length > 0) {
        if (cardSlot.classList.contains("discardable")) {
          ...
          const cardToDiscard = player.hand[slotIndex];
      ...
      if (cardSlot.classList.contains("playable")) {
        ...
        if (
          ...
          const img = cardSlot.querySelector(".card-img");
          const startRect = img ? img.getBoundingClientRect() : null;
          const cardObj = player.hand[slotIndex];

          // 2. Fire the Director action (which places the card in the pile)
          const actionPromise = Director.handleHandAction(game, slotIndex, player);
          
          // 3. Delay animation slightly so Director can render the target card in the discard pile first
          setTimeout(() => { AnimationManager.flyHandToDiscard(startRect, cardObj); }, 50);
...
async function initiateGame() {
  ...
  const savedName = localStorage.getItem("BOUNTY_PLAYER_NAME") || "PLAYER 1";

  // 3. Initialize Engine with the captured name
  game = new BountyEngine(savedName);
  initDebugKeys(game);
...
export async function playCPUTurns() {
  ...
  while (game && !game.state.gameOver) {
    ...
    if (
      ...
      const sabotageIdx = player.hand.findIndex((c) => c?.rank === 4);
      if (sabotageIdx !== -1) {
        window.isProcessingAction = true; // Lock UI
        View.render(game); // Immediately drop the "playable" fanned-out UI

        const slot = document.querySelector(`#player-${game.currentPlayerIndex} .card-slot[data-index="${sabotageIdx}"] .card-img`);
        const startRect = slot ? slot.getBoundingClientRect() : null;
        const cardObj = player.hand[sabotageIdx];

        const actionPromise = Director.handleHandAction(game, sabotageIdx, player);
      ...
      const shieldIdx = player.hand.findIndex((c) => c?.rank === 11);
      if (shieldIdx !== -1 && hand.length >= 2) {
        window.isProcessingAction = true; // Lock UI
        View.render(game); // Immediately drop the "playable" fanned-out UI

        const slot = document.querySelector(`#player-${game.currentPlayerIndex} .card-slot[data-index="${shieldIdx}"] .card-img`);
        const startRect = slot ? slot.getBoundingClientRect() : null;
        const cardObj = player.hand[shieldIdx];

        const actionPromise = Director.handleHandAction(game, shieldIdx, player);
        if (!isSimulating && startRect) {
          setTimeout(() => { AnimationManager.flyHandToDiscard(startRect, cardObj); }, 50);
        }
        await actionPromise;
    ...
    if (!isSimulating) {
      ...
      const speedMultiplier = parseFloat(localStorage.getItem("BOUNTY_SPEED")) || 1.0;
...
function evaluateCPUBounty(game, player) {
  const hand = player.hand.filter((c) => c !== null);
  const others = game.players.filter((p) => p !== player && !p.isEliminated);
  const otherHandSizes = others.map(
    (p) => p.hand.filter((c) => c !== null).length,
  );

  // Strategy for 1 card: Accept unless all others have 0 cards
  if (hand.length === 1) {
    const allOthersEmpty = otherHandSizes.every((size) => size === 0);
    return allOthersEmpty ? "DECLINE" : "ACCEPT";
  }

  // Strategy for 2 cards: Do not accept if all others have <= 1 card
  if (hand.length === 2) {
    const allOthersLow = otherHandSizes.every((size) => size <= 1);
  ...
  if (hand.length === 3) {
    if (others.length === 1) {
      const hasSabotage = others[0].hand.some((c) => c?.rank === 4);
...
function getCPUStandardGuess(rank) {
  const difficulty = localStorage.getItem("BOUNTY_DIFFICULTY") || "NORMAL";
  ...
  if (difficulty === "HARD" && game) {
    ...
    game.players.forEach((p) =>
      cardsInHands.push(...p.hand.filter((c) => c !== null)),
    );

    // Hard mode "remembers" everything currently visible on the table
    const knownCards = [...cardsInHands, game.upCard, ...game.discardPile];

    let higherPossible = 0;
    let lowerPossible = 0;

    // Calculate remaining cards in a standard 52-card deck
    for (let r = 1; r <= 13; r++) {
      // Count how many of this rank are already accounted for
      const countKnownOfRank = knownCards.filter((k) => k.rank === r).length;
      const remainingOfRank = 4 - countKnownOfRank;
...
function getCPUSocialChoice() {
  const colors = ["Yellow", "Red", "Blue", "Green"];
  return colors[Math.floor(Math.random() * colors.length)];
}

const initDebugKeys = (game) => {
  document.addEventListener("keydown", async (e) => {
    ...
    if (e.key === "7") {
      ...
      const debugSeven = { rank: 7, color: "Yellow" };
...
if (e.key === "t" || e.key === "T") {
  ...
  const debugThree = { rank: 3, color: "Yellow" };
  ...
  View.render(game);
    ...
    if (e.key.toLowerCase() === "b") {
      ...
      const debugKing = { rank: 13, color: "Yellow" };
    ...
    if (e.key === "4") {
      const player = game.players[0];
      const emptyIdx = player.hand.indexOf(null);
      const debugCard = { rank: 4, color: "Red" };
    ...
    if (e.key === "5") {
      ...
      const emptyIdx = emma.hand.indexOf(null);
      const debugCard = { rank: 4, color: "Green" };
    ...
    if (e.key.toLowerCase() === "j") {
      const player = game.players[0];
      const emptyIdx = player.hand.indexOf(null);
      const debugCard = { rank: 11, color: "Blue" };
      if (emptyIdx !== -1) player.hand[emptyIdx] = debugCard;
      else if (player.hand.length < 4) player.hand.push(debugCard);
      View.render(game);
    }
    ...
    if (e.key.toLowerCase() === "k") {
      const emma = game.players[3];
      const emptyIdx = emma.hand.indexOf(null);
      const debugCard = { rank: 11, color: "Yellow" };
      if (emptyIdx !== -1) emma.hand[emptyIdx] = debugCard;
      else if (emma.hand.length < 4) emma.hand.push(debugCard);
      View.render(game);
    }
    ...
    if (e.key.toLowerCase() === "q") {
      ...
      const debugQueen = { rank: 12, color: "Red" };
