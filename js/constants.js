...
export const TIMING = {
  ...
  REVEAL: 800, // card flips after THINKING pause. REVEAL is the pause before the result is shown.
  RESULT: 1000, // result shown after the REVEAL pause
  DISCARD_PAUSE: 300, // pause after result, before the discard banner comes, when applicable
  POST_TURN: 400, // short pause so the player can see turn result before play moves on
...
export const CARD_COLORS = ["Yellow", "Red", "Blue", "Green"];
...
export const UI_STRINGS = {
  ...
  // Queen Social Strings
  QUEEN_SOCIAL_BANNER: "Rainbow Round!",
  QUEEN_SOCIAL_SUCCESS: "Correct!",
  SOCIAL_THEME_MSG: "Theme: ", // Used as a prefix
  SOCIAL_PICKED: " picked ", // Used for "PLAYER picked COLOR"
...
// Centralized Elimination Causes
...
export const pickReaction = (type) => {
  const pool = REACTIONS[type];
  if (!pool || pool.length === 0) return "";
  return pool[Math.floor(Math.random() * pool.length)];
};

export const pickBountyReaction = (handCount) => {
