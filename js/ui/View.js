...
export const View = {
  render(game, overrideBanner = null) {
  ...
  createStrewnCardElement(card, index) {
 ...
 generateStrewnMeta(prevCard) {
  ...
  renderStatsOverlay(game) {
    if (typeof renderStatsOverlay === "function") {
      renderStatsOverlay(game);
    }
  },
  ...
  renderPlayers(players, activeIndex, game) {
  ...
  renderControls(mode, player, state, game) {
   ...
   * @param {string} type - 'theme' (Phase 1) or 'guess' (Phase 2)
   */
  createBtn(parent, text, value, className, type = "guess") {
  ...
  updateSlot(el, card, isDiscardable = false, isPlayable = false, game = null) {
    ...
    if (card) {
      ...
      const mapping = { 3: "triple-card", 4: "sabotage-card", 7: "lucky-7-card", 11: "shield-card", 12: "queen-card", 13: "king-card" };
...
<div class="stats-container" style="max-width: 520px; width: 96%; padding: 0; max-height: 95vh; overflow-y: auto; border: none; background: #f5f4f0; border-radius: 18px; box-shadow: 0 12px 40px rgba(0,0,0,0.3);">
  ...
  _createPlayerRow(p, index, isWinner, totalRounds, finalCause) {
  ...
  renderAllTimeRecordsOverlay() {
    ...
    const getAvg = (val) => (val / games).toFixed(1);
    const formatPct = (val) =>
      val === null || val === "--" ? "--" : val + "%";

    const getGlobalPct = (subKey) => {
      const stat = sub[subKey];
      if (!stat || !stat.attempts || stat.attempts === 0) return "0.0%";
      return ((stat.success / stat.attempts) * 100).toFixed(1) + "%";
    };

    const getBountyAvgSuccess = () => {
...
<div class="stats-container" style="max-width: 520px; width: 96%; padding: 0; max-height: 95vh; overflow-y: auto; border: none; background: #f5f4f0; border-radius: 18px; box-shadow: 0 12px 40px rgba(0,0,0,0.3);">
  ...
  _createSpecialistRow(label, color, bgColor, high, avg, succPct, showSucc) {
  ...
  _createComboRow(label, color, bgColor, high, avg) {
    const displayHigh = high === null || high === undefined ? 0 : high;
    const F = `font-family: 'Inter', sans-serif;`;
  ...
  renderRankAccuracyTable(records) {
...
<div style="display: flex; flex-direction: column; gap: 2px; margin-top: 4px;">`;
  ...
  _renderSessionRankTable(rankAccuracy) {
  ...
  renderEliminationSummary(records) {
    const allCauses = [
...
<div style="margin-top: 20px; margin-bottom: 20px;">
   ...
   * @param {number} playerIndex  - 0 = human, 1-3 = CPU
   * @param {string} text         - Message to display (UPPERCASE)
   * @param {boolean} pinned      - If true, bubble stays until explicitly cleared.
   * If false, auto-dismisses after ~1.8s.
   */
  showBubble(playerIndex, text, pinned = false) {
