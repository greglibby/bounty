...
export const AnimationManager = {
  async flyRectToRect(startRect, endRect, imgPath, options = {}) {
...
async flyAndFlip(startRect, endRect, faceImgPath, backImgPath, options = {}) {
    return new Promise((resolve) => {
      ...
      const innerCard = container.querySelector(".ghost-card-flip-inner");
  ...
  async flyDrawToCenter(card) {
    ...
    const targetCard = strewnCards[strewnCards.length - 1];
    ...
    if (targetCard) {
      ...
      targetWidth = targetCard.offsetWidth;
   ...
   */
  async flyTransientToHand(playerIdx, card, startRect) {
    ...
    const targetImg = Array.from(playerBox.querySelectorAll(".card-img")).find(
      (img) => img.src.includes(imgPath.replace("images/", "")),
    );
...
async flyToDiscard(startRect, card) {
    const discardSlot = document.getElementById("discard-pile-slot");
    ...
    if (targetImg) {
      targetParent = targetImg.closest(".strewn-card") || targetImg;
  ...
  async flyHandToDiscard(startRect, card) {
    const discardSlot = document.getElementById("discard-pile-slot");
    if (!discardSlot || !card) return Promise.resolve();
   ...
   */
  async flyDrawToHand(playerIdx, card) {
    const drawSlot = document.getElementById("draw-pile-slot");
