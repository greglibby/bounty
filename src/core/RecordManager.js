export const RecordManager = {
  ...
  updateRecords(currentGameStats) {
    ...
    if (currentGameStats.playerWon) records.gamesWon++;
    ...
    records.winMethods[method] = (Number(records.winMethods[method]) || 0) + 1;
    ...
    if (currentGameStats.rankAccuracy) {
      ...
      currentGameStats.rankAccuracy.forEach((data, rank) => {
        ...
        if (
          ...
          typeof records.rankAccuracyAllTime[rank].correct === "undefined"
        ) {
          records.rankAccuracyAllTime[rank] = {
            correct: 0,
            incorrect: 0,
        ...
        const r = records.rankAccuracyAllTime[rank];
        r.correct += Number(data.correct) || 0;
        r.incorrect += Number(data.incorrect) || 0;
        r.ties += Number(data.ties) || 0;
    ...
    ["ace", "sabotage", "shield", "lucky7", "queen", "bounty"].forEach(
      (key) => {
        const currentTotal = Number(s[key]?.total) || 0;
    ...
    if (currentGameStats.combos) {
      ["pair", "straight", "flush"].forEach((key) => {
    ...
    for (const [key, data] of Object.entries(currentSubResults)) {
      if (data.a > 0) {
        if (data.rate > records.subStats[key].high)
          records.subStats[key].high = data.rate;
        if (
          records.subStats[key].low === null ||
          data.rate < records.subStats[key].low
        )
          records.subStats[key].low = data.rate;

        records.subStats[key].success += Number(data.s) || 0;
        records.subStats[key].attempts += Number(data.a) || 0;
      }
    }

    // --- QUEEN ROUND LENGTH TRACKING ---
    const roundLengths = s.queen?.roundLengths || [];
    if (roundLengths.length > 0) {
      const rl = records.subStats.queenRoundLength;
      roundLengths.forEach((len) => {
