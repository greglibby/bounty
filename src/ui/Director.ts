...
import type { Card, CardColor, GameMode, GameState, Player, SpecialistResult } from "../types/index.js";
...
export const Director = {
  ...
  async playKingBountySequence(
    ...
    for (let i = 0; i < result.sequence.length; i++) {
      const step: BountySequenceStep = result.sequence[i];
  ...
  async handleDiscardPhase(game: BountyEngine): Promise<void> {
    ...
    if (isHuman) {
      ...
      while (game.state.mustDiscard.length > 0) {
        ...
        const targetRequirement: number = game.state.mustDiscard[0];
        ...
        if (actualIndex !== -1 && player.hand[actualIndex]) {
          const cardToDiscard: Card | null = player.hand[actualIndex];
  ...
  kickstartCPU(game: BountyEngine): void {
  ...
  async handleGameOver(game: BountyEngine): Promise<void> {
    ...
    const humanPlayer: Player = game.players[0];
