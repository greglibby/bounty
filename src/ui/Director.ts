...
export const Logger = {
  ...
  formatCard(card: Card | null | undefined): string {
    ...
    const strewn = card._strewn
      ? ` [${card._strewn.rot}-${card._strewn.offset}]`
      : "";
  ...
  logTurn(
    ...
    console.log(
      "%c --- PRE-TURN STATE --- ",
      "color: #ff9100; font-weight: bold; font-size: 0.85em;",
    );
    ...
    if (result && (result as SpecialistResult).flipped) {
      console.log(
        `flip card: ${this.formatCard((result as SpecialistResult).flipped)}`,
      );
    }
    ...
    console.log(
      "%c --- POST-TURN STATE --- ",
      "color: #00ff41; font-weight: bold; font-size: 0.85em;",
    );
...
export const Director = {
  ...
  async playTurn(game: BountyEngine, choice: string | number): Promise<void> {
    ...
    if (!isHuman && !isSimulating) {
      ...
      if (game.state.mode === MODES.KING_BOUNTY) {
        const activeCards: number = player.hand.filter(
          (c) => c !== null,
        ).length;
        View.showBubble(
    ...
    if (result && (result as SpecialistResult).type === "ACCEPT_START") {
      ...
      await this.playKingBountySequence(
        game,
        player,
        result as BountyAcceptResult,
      );
      return;
    }
  ...
  async playKingBountySequence(
    ...
    for (let i = 0; i < result.sequence.length; i++) {
      ...
      if (step.isMatch) {
        ...
        const failMsg: string = isLastFlip
          ? "NO MATCH! TAKE CARD."
          : "NO MATCH!";
        View.showBubble(
          game.currentPlayerIndex,
  ...
  async executeRevealSequence(
    ...
    const guessShout: string =
      guessShouts[String(choice)] || `${String(choice).toUpperCase()}!`;
    if (isHuman) {
    ...
    await AnimationManager.flyDrawToCenter(
      (result as SpecialistResult).flipped!,
    );
    SoundManager.play("flip");

    // Physical Flip (Transient Card sync)
    (game.state as AugmentedState).lastFlippedCard = (
      result as SpecialistResult
    ).flipped;
    View.render(game);
    ...
    View.renderMessageCenter(
      ...
      turnMode,
      null,
      (result as SpecialistResult).message,
    );

    const hasCombo: boolean = game.checkCombos(player);
    const activeCardCount: number = player.hand.filter(
      (c) => c !== null,
    ).length;
    ...
    if (game.state.mustDiscard.length > 0 && !game.state.gameOver) {
      if (game.state.lastResult === UI_STRINGS.COMBO_PAIR) {
        game.recordCombo("pair");
        View.showBubble(pIdx, "DISCARDING PAIR!", true);
      }
      if (game.state.lastResult === UI_STRINGS.COMBO_STRAIGHT) {
        game.recordCombo("straight");
        View.showBubble(pIdx, "DISCARDING STRAIGHT!", true);
      }
      if (game.state.lastResult === UI_STRINGS.COMBO_FLUSH) {
        game.recordCombo("flush");
        View.showBubble(pIdx, "DISCARDING FLUSH!", true);
      }
      if (
  ...
  async handleDiscardPhase(game: BountyEngine): Promise<void> {
    ...
    if (isHuman) {
      ...
      while (
        ...
        const discardData: DiscardData = await new Promise<DiscardData>(
          (resolve) => {
    ...
    if (
      ...
      if (game.state.mode === MODES.KING_BOUNTY) {
        ...
        const activeCards: number =
          game.players[0]?.hand.filter((c) => c !== null).length ?? 0;
