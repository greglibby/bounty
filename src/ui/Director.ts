...
// RESULT_AUDIO_MAP
//
// Centralized event-to-sound table. Every SpecialistResult.type that needs a
// distinct sound is listed here. The playResultAudio() method consults this map
// first; anything not listed falls back to the generic success/failure keys.
//
// To add audio for a new specialist: add its result type string here — no other
// code changes are required.
// =============================================================================

const RESULT_AUDIO_MAP: Readonly<Record<string, string>> = {
    // Ties (played before reguess prompt)
    TIE:                "tie",
    TIE_REGUESS:        "tie",
    // Mode-specific successes
    LUCKY_7_SUCCESS:    "lucky7",
    TRIPLE_COMPLETE:    "hotStreak",
    // Bounty
    BOUNTY_MATCH:       "bounty",
    ACCEPT_START:       "bounty",
    // Specialist hand-cards
    SABOTAGE_ACTIVATED: "sabotage",
    SABOTAGE_FAIL:      "incorrect",
    SHIELD_ACTIVATED:   "shield",
    // Queen Social (resolved guess)
    QUEEN_RESOLVED:     "queen",
};
...
export const Director = {
    ...
    /**
     * Centralized audio dispatcher.
     *
     * Consults RESULT_AUDIO_MAP first for an explicit type binding.
     * Falls back to "correct" / "incorrect" for any generic RESOLVE result.
     * Silent when result is falsy or in simulation mode (no audio overhead).
     *
     * Call this once per result — never scatter SoundManager.play() calls for
     * outcome sounds inside executeRevealSequence or hand-action routing.
     * To add audio for a new specialist: add its type to RESULT_AUDIO_MAP only.
     */
    playResultAudio(result: GuessResult): void {
        if (!result || isSimulating) return;
        const r = result as SpecialistResult;

        // 1. Explicit type mapping wins
        const mapped: string | undefined = RESULT_AUDIO_MAP[r.type];
        if (mapped) {
            SoundManager.play(mapped);
            return;
        }

        // 2. Generic success / failure fallback
        if (r.success) {
    ...
    async playTurn(game: BountyEngine, choice: string | number): Promise<void> {
        ...
        if (
            ...
            // Audio: dispatch via the centralized map (sabotage → "sabotage", shield → "shield")
            this.playResultAudio(result);
            Logger.logTurn(
                game,
                player,
                "HAND ACTION",
    ...
    async executeRevealSequence(
        ...
        // CINEMATIC PAUSE: hold on the guess bubble before the card flips.
        // This gives both the human and CPU a beat to "feel" the decision.
        // The second sleep(TIMING.REVEAL) below keeps the landed card visible
        // before the result phase replaces it — both pauses are intentional.
        await sleep(TIMING.REVEAL);
        ...
        // 2. RESULT PHASE
        // Audio: single dispatch via the centralized map — no scattered play() calls here.
        this.playResultAudio(result);
